...
class TestUrlopen(unittest.TestCase):
    ...
    def test_redirection(self):
        ...
        responses = [
            (302, [("Location", "http://lisyoen2.iptime.org:%(port)s/somewhere_else")],
             ""),
            (200, [], expected_response)
        ]

        handler = self.start_server(responses)
        data = self.urlopen("http://lisyoen2.iptime.org:%s/" % handler.port)
    ...
    def test_404(self):
        ...
        try:
            self.urlopen("http://lisyoen2.iptime.org:%s/weeble" % handler.port)
    ...
    def test_200(self):
        ...
        data = self.urlopen("http://lisyoen2.iptime.org:%s/bizarre" % handler.port)
        self.assertEqual(data, expected_response)
    ...
    def test_https(self):
        ...
        data = self.urlopen("https://lisyoen2.iptime.org:%s/bizarre" % handler.port, context=context)
    ...
    def test_https_with_cafile(self):
        ...
        with warnings_helper.check_warnings(('', DeprecationWarning)):
            # Good cert
            data = self.urlopen("https://lisyoen2.iptime.org:%s/bizarre" % handler.port,
                                cafile=CERT_localhost)
            self.assertEqual(data, b"we care a bit")
            # Bad cert
            with self.assertRaises(urllib.error.URLError) as cm:
                self.urlopen("https://lisyoen2.iptime.org:%s/bizarre" % handler.port,
    ...
    def test_https_sni(self):
        ...
        context = ssl.create_default_context(cafile=CERT_localhost)
        self.urlopen("https://lisyoen2.iptime.org:%s" % handler.port, context=context)
        self.assertEqual(sni_name, "localhost")

    def test_sending_headers(self):
        handler = self.start_server()
        req = urllib.request.Request("http://lisyoen2.iptime.org:%s/" % handler.port,
    ...
    def test_basic(self):
        handler = self.start_server()
        with urllib.request.urlopen("http://lisyoen2.iptime.org:%s" % handler.port) as open_url:
    ...
    def test_info(self):
        ...
        open_url = urllib.request.urlopen(
            "http://lisyoen2.iptime.org:%s" % handler.port)
        with open_url:
    ...
    def test_geturl(self):
        ...
        handler = self.start_server()
        open_url = urllib.request.urlopen("http://lisyoen2.iptime.org:%s" % handler.port)
        with open_url:
            url = open_url.geturl()
        self.assertEqual(url, "http://lisyoen2.iptime.org:%s" % handler.port)

    def test_iteration(self):
        expected_response = b"pycon 2008..."
        handler = self.start_server([(200, [], expected_response)])
        data = urllib.request.urlopen("http://lisyoen2.iptime.org:%s" % handler.port)
        for line in data:
    ...
    def test_issue16464(self):
        ...
        request = urllib.request.Request("http://lisyoen2.iptime.org:%s" % handler.port)
